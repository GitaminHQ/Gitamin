...
<div class="content-wrapper">
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form class="form-horizontal" name="IssueForm" action="{{ route('projects.issue_update', ['owner'=>$project->owner_path, 'project'=>$project->path, 'issue'=>$issue->id]) }}" role="form" method="POST" autocomplete="off">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <fieldset>
                    <div class="form-group">
                        <label class="control-label" for="issue-title">{{ trans('forms.issues.title') }}</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" name="issue[title]" id="issue-title" required value="{{ Input::old('issue.title', $issue->title) }}">
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <div class='markdown-control'>
                                <textarea name="issue[description]" class="form-control autosize" rows="8" required>{{ Input::old('issue.description', $issue->description) }}</textarea>
                            </div>
